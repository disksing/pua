      ...
      {#each draft.agentConfigs as agent, index (rowIds[index])}
        ...
        >
          <div class="settings-agent-card-head">
            ...
            <button type="button" class="settings-delete-button" title="Delete Agent" aria-label={`Delete ${agent.name || "agent"}`} onclick={() => removeAgent(index)}><Icon name="trash-2" /></button>
          </div>
          {#if open}
