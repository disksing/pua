  ...
  {:else}
    <section class="companion-card" aria-label="Activity and provider quota companion">
      ...
      <div class="companion-scroll"><div class="companion-dark">
        ...
        <div class="companion-thread-list">{#each activeList as session (session.sessionId)}<div class={`companion-thread-row ${terminalClass(session)}`} style={`--pulse-age:${Math.min(1, (Date.now() - session.lastActiveAt) / 10000)}`}><span>{session.title || session.sessionId.slice(0, 8)}</span><span>{noteForToneSlot(session.toneSlot, activityChord).name}</span></div>{:else}<span class="idle">Waiting for activity</span>{/each}</div>
